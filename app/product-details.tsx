...
import { ChevronLeft, Package, Tag, Layers, Scan, X, TrendingUp, BarChart2, Trash2 } from 'lucide-react-native';
...
export default function ProductDetailsScreen() {
  ...
  const fetchData = async () => {
    try {
      ...
      const buyPricePerUnit = ppu > 1 ? product.buyPrice / ppu : product.buyPrice;

      setForm({
        name: product.name,
        sku: product.sku || '',
        buyPrice: ppu > 1 ? product.buyPrice.toString() : buyPricePerUnit.toString(),
        salePrice: product.salePrice.toString(),
        minStockLevel: product.minStockLevel.toString(),
        unit: product.unit || 'pcs',
        piecesPerUnit: ppu.toString(),
        description: product.description || '',
        categoryId: product.categoryId,
      });

      if (ppu > 1 && !WEIGHT_UNITS.includes(product.unit)) {
        setPriceMode('box');
        const boxes = Math.floor(product.stock / ppu);
        const pieces = product.stock % ppu;
        setStockEntry({ boxes: boxes.toString(), pieces: pieces.toString() });
      } else {
        setPriceMode('single');
        setStockEntry({ boxes: '', pieces: product.stock.toString() });
      }
  ...
  const handleUpdate = async () => {
    ...
    setSaving(true);
    try {
      const finalBuyPrice = priceMode === 'box' ? buy : buy * ppu;
      ...
      await api.put(`/products/${id}`, {
        ...
        buyPrice: priceMode === 'box' ? buy : buy, // Box price or Single price logic correctly handled backend side? Wait, backend stores raw buyPrice.
        salePrice: sale,
        stock: finalStock,
      ...
      <ScrollView contentContainerStyle={styles.form} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.stockBox}>
            {isWeightUnit ? (
              <View style={styles.fieldGroup}>
                <Text style={styles.label}>Current Weight ({form.unit})</Text>
                <TextInput
                  style={styles.input}
              ...
              <View style={styles.twoCol}>
                ...
                <View style={{ flex: 1, marginLeft: priceMode === 'box' ? 12 : 0 }}>
                  <Text style={styles.label}>
                    {priceMode === 'box' ? 'Loose Pieces' : 'Current Piece Count'}
                  </Text>
                  <TextInput
                    style={styles.input}
                    value={stockEntry.pieces}
